...
Considering narrowband signals, this choice of weights may be interpreted as a generalized version of the \emph{conventional beamformer} (CB) \cite{van2004optimum} which is a mere time shift (phase shift in narrowband signals) of the input signals as if the impinging wave is coming from the array's boresight direction (i.e. $\theta_{d}=\pi/2$) as illustrated in Fig.\ref{fig_CB_vanTrees}.
\begin{figure}[t!]
    \begin{center}
    ...
    The \emph{conventional beamformer} - time shifting the input signals as if the impinging wave from a certain DOA arrives from the array's boresight.
    }
